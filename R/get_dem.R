#' Get episodes of democratization (opening, democratic deepening)
#'
...
get_dem <- function(data = vdemdata::vdem,
  ...
  full.df <- data %>%
    ...
    dplyr::mutate(dem_ep_start_year = ifelse(!is.na(episode_id),first(year) +1, NA),
                  ...
                  censored = ifelse(dem_ep==1 & (dem_ep_end_year > codingend-1 | dem_ep_end_year > gapstart1-1 | dem_ep_end_year > gapstart2-1 | dem_ep_end_year > gapstart3-1) & manifest ==1, 1, NA), # we check whether the episode is still ongoing (=censored)
                  ...
                  dem_ep_aut = ifelse(!is.na(character_id) & manifest == 1, ifelse(pre_regime <=1, 1, 0),0), # identify manifest opening episodes starting in autocracies
    ...
    ### opening episodes ###
    # assess the type of opening episode: success, failure
    group_by(country_id) %>%
    ...
    ungroup() %>%
    group_by(dem_ep_id) %>%
    dplyr::mutate(founding_elec = min(hablar::s(ifelse(v2x_regime > 1 & dem_ep_aut ==1 & v2elasmoff_ord > 1 & (v2eltype_0 == 1 | v2eltype_4 ==1 | v2eltype_6 ==1) & lag(v2x_regime > 1, n=1), year, NA))), # let's find the year of the first election after transition (=founding election)
    ...
    dplyr::mutate(dem_ep_end_year = ifelse(success == 1, last_trans, dem_ep_end_year),  # let's remove stasis years within successful episodes (for all columns below...)
                  ...
                  max_trans = max(hablar::s(ifelse(reg_trans == 1,year,NA))), # here, we need the last transition within one episode, not followed by a founding election
                  fail_preem = ifelse(dem_ep_aut == 1 & success == 0 & !is.na(max_trans) & is.na(censored), 1, 0),
                  dem_ep_end_year = ifelse(fail_preem == 1, max_trans, dem_ep_end_year),  # let's remove stasis years within preempted opening episodes (for all columns below...)
                  dem_ep_id = ifelse(fail_preem ==1, paste(country_text_id, dem_ep_start_year, dem_ep_end_year, sep = "_"), dem_ep_id),
    ...
    # extract the failed opening
    # for this, the following conditions need to be meet: first, a country moves back to closed autocracy
    group_by(country_id) %>%
    dplyr::mutate(back_closed = ifelse(lead(v2x_regime, n=1L) == 0 & v2x_regime > 0 & year == dem_ep_end_year, 1, 0),
                  ...
                  last_ch_year = ifelse(v2x_polyarchy-lag(v2x_polyarchy, n=1)>=start_incl , year, NA)) %>%  # let's locate the last year of a positive change before encountering either a drop or stasis
    group_by(dem_ep_id) %>%
    dplyr::mutate(last_ch_year = ifelse(dem_ep_aut == 1, max(last_ch_year, na.rm = T), NA)) %>%
    group_by(country_id)

  # yearly drop
  year_drop <- list()
  for (i in 1:tolerance) {
    year_drop[[i]] <- ifelse(full.df$year == full.df$last_ch_year & lead(full.df$country_id, n=i)==full.df$country_id, lead(full.df$v2x_polyarchy, n=i-1)-lead(full.df$v2x_polyarchy, n=i), NA)
  }

  df1 <- do.call(cbind, lapply(year_drop, data.frame, stringsAsFactors=FALSE))
  names <- paste0('year', seq(1:10))
  colnames(df1) <- names
  year_drop <- df1 %>%
  ...
  cum_drop <- list()
  for (i in 1:tolerance) {
    cum_drop[[i]] <- ifelse(full.df$year == full.df$last_ch_year & lead(full.df$country_id, n=i)==full.df$country_id, lead(full.df$v2x_polyarchy, n=i)-full.df$v2x_polyarchy, NA)
  }

  df <- do.call(cbind, lapply(cum_drop, data.frame, stringsAsFactors=FALSE))
  names <- paste0('cum', seq(1:10))
  colnames(df) <- names
  cum_drop <- df %>%
  ...
  full.df <- full.df %>%
    ...
    # finally, we have all conditions to check for failed opening (the two created above combined by logical OR):
    group_by(dem_ep_id) %>%
    dplyr::mutate(fail_rev = ifelse(success == 0 & fail_preem == 0 & dem_ep_aut == 1 & (back_closed == 1 | cum_drop == 1 | year_drop == 1 | max(hablar::s(v2x_regime)) == 0),1,0),
                  ...
                  fail_stab = ifelse(dem_ep_aut == 1 & success == 0 & fail_preem == 0 & fail_rev == 0, 1,0)) %>%


    # now we remove any remaining stasis years
    dplyr::ungroup() %>%
    dplyr::mutate(dem_ep_end_year = ifelse((fail_rev == 1 | fail_stab ==1), last_ch_year, dem_ep_end_year),
                  dem_ep_id = ifelse((fail_rev == 1 | fail_stab ==1), paste(country_text_id, dem_ep_start_year, dem_ep_end_year, sep = "_"), dem_ep_id),
                  dem_ep = ifelse(year>dem_ep_end_year & (fail_rev == 1 | fail_stab ==1), 0, dem_ep),
                  dem_ep_aut = ifelse(year>dem_ep_end_year & (fail_rev == 1 | fail_stab ==1), 0, dem_ep_aut),
                  manifest = ifelse(year>dem_ep_end_year & (fail_rev == 1 | fail_stab ==1), 0, manifest),
                  fail_rev = ifelse(!is.na(dem_ep_id), ifelse(year>dem_ep_end_year, 0, fail_rev), 0),
                  fail_stab = ifelse(!is.na(dem_ep_id), ifelse(year>dem_ep_end_year, 0, fail_stab), 0),
                  dem_ep_id = ifelse(year>dem_ep_end_year, NA, dem_ep_id)) %>%
