#' Get episodes of democratization (liberalization, democratic deepening)
#'
...
get_dem <- function(data = vdemdata::vdem,
  ...
  full.df <- data %>%
    ...
    dplyr::mutate(dem_ep_start_year = ifelse(!is.na(episode_id),first(year) +1, NA),
                  ...
                  dem_ep = ifelse(dem_ep==1 & max(v2x_polyarchy, na.rm = T) - min(v2x_polyarchy, na.rm = T) > cum_incl, 1, 0), # we check whether the cumulated change in each potential episode was substantial (> cum_inc)
                  ...
                  dem_ep = min(ifelse(pre_ep_year==1 & year-lead(year, n=1) < -1, 0, dem_ep)),
                  dem_ep_aut = ifelse(!is.na(character_id) & dem_ep == 1, ifelse(pre_regime <=1, 1, 0),0), # identify manifest autocratic adaptation episodes starting in autocracies
                  dem_ep_dem = ifelse(!is.na(character_id) & dem_ep ==1, ifelse(pre_regime >1, 1, 0),0), # identify manifest democratic deepening episodes starting in democracies
    ...
    ungroup() %>%
    dplyr::mutate(dem_ep_end_year = ifelse(success == 1 & censored==0, last_ch_year, dem_ep_end_year),  # let's remove stasis years within successful episodes (for all columns below...)
                  dem_ep_id = paste(country_text_id, dem_ep_start_year, dem_ep_end_year, sep = "_"),
                  dem_ep = ifelse(year>dem_ep_end_year, 0, dem_ep),
    ...
    dplyr:: mutate(max_trans = max(hablar::s(ifelse(reg_trans == 1,year,NA))), # here, we need the last transition within one episode, not followed by a founding election
                   ...
                   dem_ep_end_year = ifelse(fail_preem == 1, last_ch_year, dem_ep_end_year)) %>% # let's remove stasis years within preempted episodes (for all columns below...)
    ungroup() %>%
  ...
  full.df <- full.df %>%
    ...
    dplyr::mutate(sub_dem_ep_id = ifelse(dem_ep_dem == 1 & success == 0, dem_ep_id, NA),
                  ...
                  sub_dem_ep_id = ifelse(dem_ep_aut == 1 & success == 1, paste(country_text_id, dem_ep_start_year, founding_elec, sep = "_"), sub_dem_ep_id)) %>%
    dplyr::filter(dem_ep==1) %>%
    group_by(sub_dem_ep_id) %>%
    dplyr::mutate(manifest = ifelse(dem_ep==1 & max(v2x_polyarchy, na.rm = T) - min(v2x_polyarchy, na.rm = T) > cum_incl, 1, 0))%>%
